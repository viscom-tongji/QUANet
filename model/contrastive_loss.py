...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, patch_embedding, img_embedding, gt_text_embedding_map, noise_text_embeddings, gt_density):
        ...
        #print(density_mask.shape)
        density_mask = density_mask > 0. # (B, 1, 14, 14)
       # density_mask1=density_mask.int()
       # print(density_mask1.shape)
        density_mask = density_mask.permute(0, 2, 3 ,1) # (B, 14, 14, 1)
        gt_text_embedding_map = gt_text_embedding_map.unsqueeze(1).expand(-1, 14, 14, -1) #(B, 1, 1, 512)->(B, 14, 14, 512)
        #print(daa)
